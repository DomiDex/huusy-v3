...
export default function SaleTypePage() {
  ...
  useEffect(() => {
    async function fetchSaleTypeAndProperties() {
      try {
        ...
        if (!saleTypeData) {
          setError('Sale type not found');
          return;
        }

        setSaleTypeName(saleTypeData.title);
                ...
                {properties.length === 0 && (
                  <div className='col-span-full text-center py-12 bg-primary-50 rounded-lg'>
                    <p className='text-primary-800'>
                      No {saleTypeName.toLowerCase()} properties found matching
                      your criteria.
                    </p>
                  </div>
                )}
